 ...
 */
constructUri(callback) {
  let getCallOptions = { ...this.uri };
  if (query) {
    getCallOptions.uri = `/api/now/table/${serviceNowTable}` + '?' + query;
  } else {
    getCallOptions.uri = `/api/now/table/${serviceNowTable}`;
  }
}
 ...
 */
isHibernating(callback) {
  let getCallOptions = { ...this.isHibernating };
  getCallOptions.isHibernating = response.body.includes('Instance Hibernating page')
...
processRequestResults(callback) {
  let getCallOptions = { ...this.data };
  let getCallOptions = { ...this.error };
  /**
   ...
   */
   ...
   if (isHibernating(checkHibernate)) {
       console.error('Service Now is hibernating');
       this.error = 'Service Now is hibernating';
   } else if (error) {
       console.error(callbackError);
       this.error = 'There was an error in the request';
   } else {
       this.data = {
           response,
       };
...
sendRequest(callback) {
  ...
  if (getCallOptions.query)
    getCallOptions.uri = constructUri(getCallOptions.serviceNowTable, getCallOptions.query);
  else
    getCallOptions.uri = constructUri(getCallOptions.serviceNowTable);
  /**
